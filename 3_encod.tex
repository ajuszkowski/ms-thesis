...
While working on the \porthos[2], we have applied some modifications to the encoding scheme for the control-flow.
...
Equation~\ref{enc:cf:br} shows the encoding for the branching control-flow depicted in Figure~\ref{encode:cf:br}, that considers both transitions and guards.
 %(similarly to the sequential control-flow)  %$\{\emptyset, \; (e_1), \; (e_1\,\Rightarrow\,e_2), \; (e_1\,\Rightarrow\,e_3)\}$.
...
Note that the sequential control-flow is a special case of branching with the only transition guarded by $\epsilon$ (that is encoded as \texttt{true}).
