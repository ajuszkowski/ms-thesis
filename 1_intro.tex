...
Although the parallelisation of the system can improve its performance drastically, it opens numerous of problems connected to correctness, robustness and reliability, which makes the concurrent program design one of the most difficult problems of programming~\cite{mckenney2017parallel}.
...
For example, consider a shared memory concurrent system that executes two parallel processes as it is described in Figure~\ref{simple_wmm_x86}.
The process~\texttt{P0} writes value~\texttt{1} to the shared variable~\texttt{x} and reads the value of shared variable~\texttt{y}, and the process~\texttt{P1} writes the~\texttt{y} and reads the~\texttt{x}.
...
However, this state is reachable on the x86 architecture as each processor may cache the write to shared memory location into its local write buffer, so that they do not immediately become visible by processes running on other cores.
This behaviour is known as \textit{store buffering (SB)}.
The little code examples that illustrate specific behaviour of an execution environment are called \textit{litmus tests}.
