...
\begin{outline}
  ...
  \1 \texttt{XControlFlowEvent} \\
    ...
    In other words, we suppose that the knowledge base of \porthos{} is complete and the tool can resolve all memory-operation functions and fence instructions.
...
A \wmodel{} represents a recursively defined AST of relations defined by the memory model and an assertion expression.
...
The \wmodel{} is constructed by the converter \texttt{Cat2WmodelConverter}, which calls \texttt{Cat2WmodelConverterVisitor}, the stateless visitor of memory-model ANTLR syntax tree. %TODO: Implement this!!!
Currently, the visitor supports only the declarative subset of the \cat{} language, and does not process the functional-style definitions (it can to be supported by implementing the 
...
builder: filling in + verification
...
The construction of a \zformula{} is performed by the \texttt{ZformulaBuilder}.%TODO: check this class existence
...
Since the C language allows usage of variables that have the same name to be declared in nested contexts, the \texttt{XMemoryManager} carries the stack of block block-contexts for local variables. %TODO: refactor XMemoryManager!
