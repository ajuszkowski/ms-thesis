...
\begin{outline}
  ...
  \1 \texttt{XComputationEvent} \\
    ...
    \newline
    Note, for the purpose of simplifying the X abstraction level, computation events may be modelled as a bodiless function invocation (for instance, `\lstinline{x + y}' is the invocation of the function `\lstinline{+}' to argument `\lstinline{x}' and `\lstinline{y}').
...
\begin{itemize}[noitemsep]
  \item control-flow edges: a \textit{primary} edge (epsilon or if-then transition) and an \textit{alternative} edge (if-else transition), see more in Section~\ref{ch:port:enc:cf};
  \item data-flow edges: the \co-relation edge and the \rf-relation edge. %TODO! implement and then describe more!
\end{itemize}
...
The precompiler traverses the \ytree{} and collects information necessary for compiling it into an \xgraph{}.
...
In addition, \porthos[2] supports ... int\_t .... 

   %TODO: support this.

%TODO: describe the XType
%TODO: definitely get rid of bit32
...
%TODO: <not implemented>
The type analysis includes the function resolution: for correct resolving the semantics of functions, the compiler needs a mapping of each function invocation to its signature.
...
- size\_t. ...%https://en.wikipedia.org/wiki/C_data_types#stddef.h


\paragraph{Variables kinds determination}
\label{ch:impl:proc:x-pre-compiler:var}

As it was discussed in Section~\ref{ch:impl:model:ytree}, the \ytree{} encodes all named objects as \texttt{YVariableRef}.
