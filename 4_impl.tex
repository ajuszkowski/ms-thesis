...
%
\footnote{For ensuring this, one merely has to look at existing C compilers, for instance, the open-source \tool{gcc} compiler, which uses the C parser written in more than 18.5 thousand lines of code (see \url{https://github.com/gcc-mirror/gcc/blob/master/gcc/c/c-parser.c})}%
%
...
In \porthos[2], we clearly separated the parser (generated from the grammar file `\textit{<grammar>.g4}') from the converting the \tool{ANTLR} syntax tree to the AST, that is one for all languages of an input program.
...
%The \xgraph{} is constructed by the mutable graph builder. %TODO: name of builder -- after refactoring
...
A \wmodel{} represents a recursive AST of \textit{computations over relations} and assertion expressions defined by the memory model.
...
<todo: enumerate elements -- after completing>
...
`\texttt{x = 1 * (2 + 3)}' would be compiled to two consequent events `\texttt{eval(2 + 3); write(register <- 1 * (2 + 3));}' instead of single event `\texttt{write(register <- 1 * (2 + 3));}').
If the computation event was not used at all (for example, in the following C code: `\texttt{foo(); 1; bar();}' the execution event `\texttt{eval(1)}' is not used by the model), it is also removed from the event-graph (see justification in Section~\ref{ch:impl:model:xgraph:evt}).
...
The context is a data structure that carries the \textit{state} and some additional information in case of processing non-linear statements (e.g., conditional event, first and last then- and else-branch events for binding, etc.).
...
\item \texttt{WaitingNextLinearEvent}: the standard interpreter state for processing next linear event,
...
For simplicity of the interpretation, we keep the jump events (no-computation events) in the flow-graph, however they can be removed and the edges to and from them can be rewritten if necessary.
...
The Figure~\ref{fig:loop-unroll} illustrates unrolling for the cyclic control-flow graph on the left-hand side (the square node $S_{+}$ denotes the complete sink node, while the $S_{-}$ -- incomplete).
...
\node[rectangle,draw] (6inc) [below left=1cm and 0.6cm of 16] {$S_{-}$};
\node[rectangle,draw] (6com) [below of=6inc] {$S_{+}$};
...
(26) edge [bend right=20] node {} (6com)
(36) edge [] node {} (6inc)
(16) edge [] node {} (6inc)
...
Each next event increments the unrolling depth counter (the \textit{event reference-ID}), the integer that is stored by the \texttt{XEvent} thus making it \textit{an event reference} on non-zero values.
...
During the work on \porthos[2], we tested different schemes of loop unrolling.
\\<TODO: Say sth about these schemes + pictures>
...
This is a structure that contains the result of analysis and auxiliary information such as collected errors and time of execution.
