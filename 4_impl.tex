...
Thereafter, \wmodel{} and acyclic \xgraph{} are encoded (Section~\ref{ch:impl:proc:z-encoder}) to a \zformula{} representation (a wrapper over an SMT logical formula), which then is translated to an SMT-formula (Section~\ref{ch:impl:proc:smt-transator}), which then is solved by the SMT-solver (Section~\ref{ch:impl:proc:smt-solver}).
