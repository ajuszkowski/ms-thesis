...
`\texttt{x = 1 * (2 + 3)}' would be compiled to two consequent events `\texttt{eval(2 + 3); write(register <- eval(1 * eval(2 + 3)));}' instead of single event `\texttt{write(register <- eval(1 * eval(2 + 3)));}').
...
Figure~\ref{fig:loop-unroll} illustrates unrolling for the cyclic control-flow graph on the left-hand side (the square node $S_{+}$ denotes the complete sink node, while the $S_{-}$ -- incomplete).
...
During the work on \porthos[2], we have tested different schemes of loop unrolling.
...
This includes ... <todo: recall said before>
