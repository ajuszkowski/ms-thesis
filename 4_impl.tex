...
\begin{outline}
  ...
  \1 \texttt{XComputationEvent} \\
    We distinguish two types of computation events:
      ...
      %\end{itemize}
    Since a computation event performs computation over local-only memory and does not change value of any memory unit, the \textit{computation} abstraction (as the CPU time spent for the computation itself) can be safely removed from the model, and the computation event can be seen as a simultaneous operation.
    Therefore the computation event class implements both \texttt{XEvent} and \texttt{XMemoryUnit}.
  ...
  \1 \texttt{XControlFlowEvent} \\
    ...
    %\begin{itemize}[noitemsep]
      ...
      \\
      If the method has not been resolved, \textit{TODO: think about it. Will the formula be SAT? Or do we need to abort the analysis?}
    %\end{itemize}
...
\end{lstlisting}
\end{multicols}
\caption{Sketch of the visitor pattern}
\label{fig:visitor}
\end{figure}
...
The preprocessor traverses the \ytree{} and collects information necessary for compiling it into an \xgraph{}. 
