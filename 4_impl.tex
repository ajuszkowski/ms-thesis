...
As its predecessor, \porthos[2] uses the open-source SMT solver \tool{Z3} by Microsoft Research~\cite{de2008z3}. However, unlikely its predecessor, the \porthos[2] has an additional abstraction level \zformula{} (see Section~\ref{ch:impl:model:zformula}) that allow to use any other SMT solver. %TODO: *should* allow?
...
Thereafter, \wmodel{} and acyclic \xgraph{} are encoded (Section~\ref{ch:impl:proc:z-encoder}) to a \zformula{} representation (a wrapper over an SMT logical formula), which then is translated to an SMT-formula (Section~\ref{ch:impl:proc:smt-converter}), which then is solved by the SMT-solver (Section~\ref{ch:impl:proc:smt-solver}).
...
The semantics of the methods also is being resolved during the pre-compilation stage via the \textit{invocation hooking} mechanism (see Section~\ref{ch:impl:proc:x-pre-compiler:hooks}).
...
Also, current version of \porthos[2] simply ignores C processor directives, however, in future it can be extended to support them.
...
that is easily convertable to an SMT type (modelled as \texttt{ZType}).%TODO: add the class ZType

The memory units are crated and stored by the stateful \texttt{XMemoryManager}, which provides interface for accessing memory units during compilation stage.
The memory manager takes into account the higher priority of local variables over global variables specific for C language.
The \texttt{XMemoryManager} provides interface for registering shared variables at the precompilation stage, registering local variables dynamically at the compilation stage, and returning memory unit instances for the requests by name.
\textit{TODO: context stack}%TODO: refactor XMemoryManager!
...
\begin{outline}
  ...
  \1 \texttt{XComputationEvent} \\
    ...
    \newline
    Note, for the purpose of simplifying the X abstraction level, computation events may be modelled as a bodiless function invocation (for instance, `\lstinline{x + y}' is the invocation of the function `\lstinline{+}' to the arguments `\lstinline{x}' and `\lstinline{y}').
  ...
  \1 \texttt{XControlFlowEvent} \\
    ...
    %\begin{itemize}[noitemsep]
      ...
      \2 \texttt{XMethodCallEvent} that models the method call with the \texttt{FASTCALL} calling convention (passing arguments in registers).
      %TODO: check the diagrams if the XMCE really inherits XLMU
      The method call also implements the \texttt{XLocalMemoryUnit} since it represents the computed value returned by the method call.
      If the method has been resolved, the actual arguments are bind to the formal parameters (treated as temporary local memory units), the called \texttt{XMethodCallEvent} is pushed onto the call stack, and the execution jumps to the method body.
      Each \texttt{return} statement creates the assignment of the method call event on the top of call stack.
      \\
      If the method has not been resolved, \textbf{TODO: think about it. Will the formula be SAT? Or do we need to abort the analysis?}
    %\end{itemize}
...
Once being constructed, graph must conform the following requirements:
\begin{enumerate}
...
The operator instance, instead of having a single method that handles and element of the instance, it extends the visitor interface and splits the handler method into multiple methods, one for each type of element.
...
%The ANTLR grammar for the C preprocessor may be found 
...
The ANTLR grammar for the \cat language was extracted from the parser used by \tool{herd} tool%
...
Currently, \porthos[2] operates only the primitive C types (such as \texttt{int}, \texttt{char}, \texttt{float}, etc.), which is modelled as an enumeration \texttt{XType}. %TODO: model it.
...
For resolving the type aliases, the precompiler should make an extra traverse of the \ytree{} before the precompilation stage and build up a symbol map.

%TODO: <not implemented>
The type analysis includes the process of resolving semantics of function invocations.
The result is stored in a map of function invocation expression to its resolved signature, so that the compiler can decide either to jump to the function body and interpret it, or to hook the invocation if the semantics of the function is resolved.
...
The X-type construction: //code of class YType2TypeConverter (todo: rename: it's not conversion, it's a type construction, from the y-level name+qualifiers+
...
On the compilation stage, once the compiler meets the reference to a variable, it should know whether it is local or global variable.
In general case, 
...
- parameters of process-functions (parameters of other funcs are local)

- EXPORT\_SYMBOL "exports the symbol for dynamic linking"

-EXPORT\_SYMBOL\_GPL %https://lwn.net/Articles/154602/

 %https://stackoverflow.com/questions/6670589/how-to-prevent-error-symbol-undeclared-here-despite-export-symbol-in-a-linu
...
converting recursive structure to plain
...
stateful. Managers, [graph] builders
  interfaces of XMemoryManager: listing, paragraph
