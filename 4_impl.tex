...
For example, the C11 sequentially consistent load operation `\lstinline{l.store(memory_order_seq_cst, r)}' whould be compiled for the \texttt{x86} architecture as the write instruction `\lstinline{MOV l, r}' followed by the fence instruction `\lstinline{MFENCE}', whether for the \texttt{POWER} architecture this code would be compiled as the `\lstinline{hwsync}' followed by the store instruction.
