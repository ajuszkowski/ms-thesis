...
\begin{outline}
  ...
  \1 \texttt{XControlFlowEvent} \\
    ...
    For interpreting known function invocations, \porthos[2] must perform three steps: bind actual arguments to the formal parameters (treated as temporary local memory units), push the called \texttt{XMethodCallEvent} onto the call stack, and the perform the control-flow jump to the function body.
...
%Note that the same mechanism (with several modifications) can be used for processing assembly language calls.
An invocation hook can model any type of operations (memory, fence, computational, etc.).
%For instance, the \texttt{XLegacyInvocationHook} intercepts calls \lstinline{`location.store(atomic, register)'} and processes the store operation with respect to the specified atomic.
For instance, the \texttt{XKernelInvocationHook} intercepts the call \lstinline{`WRITE_ONCE(dst_shared, src_local)'} and replaces the invocation with its hook action shown in Figure~\ref{code-invoc-hook}.
%and processes the store operation with respect to the following .
...
\end{lstlisting}
\caption{Example of the invocation hook for intercepting the Linux kernel-specific functions}
\label{code-invoc-hook}
\end{figure}
...
%TODO: say that program is the set of processes , but the implements the same interface !!! it's not clear here.

%TODO: !!! Say that interpreter does not know anyth about Y, all its interface methods are <sth> -> <X>. The only part that know about Y is Y2Xvisitor
...
The X-program interpreter is invoked by the \texttt{Y2XConverterVisitor} which walks down the \ytree{} recursively.
...
If the computation event was not used at all (for example, in the following C code: `\texttt{foo(); 1; bar();}' the execution event `\texttt{eval(1)}' is skipped by the model), it is also removed from the event-graph (see justification in Section~\ref{ch:impl:model:xgraph}).
...
As an example of usage the interpreter, consider the code of the \texttt{Y2XConverterVisitor} for processing \texttt{if-then-else} statement of the \ytree{} in Figure~\ref{TODO}.
...
Note that recursive function calls create unconditional back-jumps in the event-flow graph, which will also be recognised as a loop and be affected by the unrolling algorithm.
...
The \zformula{} encoder accepts as input the program and the input memory models.
...
%todo: currenlty not implemented, just converted to the old
%TODO: say more!
