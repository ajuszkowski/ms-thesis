...
For instance, the \fr-relation is defined as a sequence of inverted \rf-relation and \co-relation: $\mathr{fr} = (\mathr{rf}^{-1};\mathr{co})$. Figure~\ref{example:x86-cat} contains an example of x86-TSO weak memory model definition in \cat{} language that asserts acyclicity of communication relation, \rel{po-loc} relation, \rel{mfence} relation and some other derived relations~\cite{owens2009better}:
...
\end{figure}
\footnotetext{The \cat{}-memory model for x86-TSO can be found in official repository of \tool{herd} tool: \url{https://github.com/herd/herdtools7}}
%\begin{figure}[h]
%\begin{lstlisting}
%...
%let com = rf | fr | co
