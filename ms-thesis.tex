...
\newcommand{\KEYWORDS}{Software verification, bounded reachability analysis, weak memory models, portability analysis}
...
Moreover, the architecture of a modern shared-memory multi-core processor or optimisations performed by the compiler can cause new program behaviours, unexpected from the point of view of traditional concurrency.
The guarantees that an execution environment can provide to a programmer are described by its \textit{weak memory model (WMM)}.
%The formalisation of weak memory models
Over the last decade, weak memory models were formally defined for numerous hardware architectures and programming languages.
%Thus, the memory model-aware software analysis 
%This opens/brings new 
This opens new challenges for developing new and adapting existing methods for software verification with respect to a memory model.
...
%finite-state system
The first tool that analyses the \textit{portability} of the concurrent program from one platform to another is \textit{\porthos{}}~\cite{Porthos17a} released in April~2017.
...
Current thesis aims to extend the input language of \porthos{} so that it is able to process real-world C programs.
However, current implementation of \porthos{} seems to be hardly extensible, which implies redesign of the whole architecture of the tool in order to increase its robustness, transparency, efficiency and extensibility.
The result of the work is \textit{\porthos[2]}, a generalised framework for SMT-based memory model-aware analysis.
...
\input{96_apx_x86cat.tex}
\input{95_apx_interpreter.tex}
