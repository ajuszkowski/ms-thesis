...
\item \textit{Extending the knowledge base} of domain-specific functions may be considered as the main future research direction.
...
However, as the Z3 solver supports the theory of constant-size arrays, it might be relatively easy to extend the support for constant-size arrays (declared as `\lstinline{int arr[10];}'), enumerations and structures.
...
\item As it was mentioned in Section~\ref{ch:enc:bmc:cf} devoted to the encoding for the control-flow of the program, the new encoding scheme used by \porthos[2] allows to analyse partial functions.
Although the current \porthos[2] interpreter infrastructure does is not configured to perform a partial analysis, it can be easily supported.

\item During the implementation of \porthos[2], we intentionally skipped the time-consuming stage of writing the test suites for the tool.
Thus, in order to increase the robustness and stability of \porthos[2], we need to cover its code by unit- and functional tests.
